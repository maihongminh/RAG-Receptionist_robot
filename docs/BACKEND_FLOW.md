...
- RAG build có `--mode full` và `--mode incremental`; incremental dùng `robo_rag.index_manifest` để chỉ re-index document mới/đổi hash và xóa stale point.
...
- Đã có `/auth/change-password`, password reset token foundation, Account admin UI/API cơ bản và login rate-limit in-memory.
- `auth` mock trong request là dev-only path, mặc định bị bỏ qua.
- Chưa có email/SMS/OTP delivery thật và account admin tạo/sửa role nâng cao.
